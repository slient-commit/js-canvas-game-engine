class Drawer {
    ...
    /**
     * Draw a sprite rotated around its pivot point
     * @param {Sprite} sprite
     * @param {Vec2} position
    ...
    drawRotated(sprite, position, angle, opacity = 1) {
        ...
        this.ctx.save();
        this.ctx.globalAlpha = opacity;
        this.ctx.translate(position.X + pivot.x, position.Y + pivot.y);
        this.ctx.rotate(angle);
        this.ctx.drawImage(sprite.image, -pivot.x, -pivot.y, sprite.width, sprite.height);
        this.ctx.restore();
        return true;
    }

    /**
     * Draw a sprite with scaling around its pivot point
     * @param {Sprite} sprite
     * @param {Vec2} position
     * @param {number} scaleX
     * @param {number} scaleY
